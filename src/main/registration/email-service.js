...
export function parseProvidedEmailLines(data) {
    return data
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
...
export class ProvidedEmailService {
    account;
    hosts;
    address = '';
    host = '';
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                for (const msg of messages) {
                    ...
                    if (Number.isFinite(msgTime) && msgTime + 10000 < baseline)
                        continue;
...
export function parseOutlookLines(data) {
    const accounts = [];
    data = data.trim();
    if (!data)
        return accounts;
...
export async function refreshOutlookToken(acc) {
...
export async function getInboxCount(acc) {
...
export async function waitForOTP(acc, beforeCount, timeout, interval, abortCheck) {
...
export class DuckDuckGoEmailService {
    static QUACK_URL = 'https://quack.duckduckgo.com/api';
    authToken; // Bearer token from DDG account
    gmailAccount;
    address = '';
    baselineAwsUid = '0';
